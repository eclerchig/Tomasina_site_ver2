    ...
    <div class="col-1"> </div>	
      ...
      <select name="ID_cat" class="form-select form-select-sm" aria-label=".form-select-sm">
        <option selected disabled>Выберите котика</option>
          ...
          { 
            $breed = mysqli_fetch_assoc(mysqli_query($connect,"SELECT Name FROM breeds join cats ON breeds.ID = cats.IDПорода WHERE cats.ID = {$row['ID']}"));
            $color = mysqli_fetch_assoc(mysqli_query($connect,"SELECT Name FROM coatcolor join cats ON coatcolor.ID = cats.IDОкрас WHERE cats.ID = {$row['ID']}"));

            ?>

            <option value="<?php echo $row['ID'] ?>"><?php echo "{$row['Кличка']} Возраст: {$row['Возраст']} лет Окрас: {$color['Name']}" ?></option>
      ...
      { 
        ...
        <div class="row mt-2">
          ...
          <div class="mb-1 offset-1 col-6">
            <label for="exampleFormControlInput1" class="form-label">Порода</label>
            <input class="form-control" type="text" placeholder="<?php echo $breed['Name']?>" aria-label="readonly input example" readonly>
          </div>
        </div>
        <div class="row mt-2">
        ...
        <div class="row mt-2">
          <div class="mb-1">
            ...
            <input class="form-control" type="text" placeholder="<?php echo $color['Name']?>" aria-label="readonly input example" readonly>
          </div>
          <div class="mb-1">
